-- source: https://github.com/Rerumu/FiOne
-- modified a bit
...
local function run_lua_func(state, env, upvals)
    ...
    local op_funcs = {
        ...
        [15] = function(inst)
            ...
            return true, table.pack(memory[A](table.unpack(memory, A + 1, A + params)))
        end,
        [16] = function(inst)
            --[[DIV]]
            local lhs, rhs
        ...
        [21] = function(inst)
            ...
            return true, table.move(memory, A, A + len - 1, 1, {})
        end,
        [22] = function(inst)
            --[[CONCAT]]
        ...
        [34] = function(inst)
            ...
            memory[A] = tonumber(memory[A]) - step
            memory[A + 1] = tonumber(memory[A + 1])
            memory[A + 2] = step
	...
	while true do
        ...
        local inst_func = op_funcs[inst.op]
        local returned, values = inst_func(inst)
        if returned then
